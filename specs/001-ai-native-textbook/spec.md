...
- Q: Should guest users have access to all features or must they register to access personalization and chatbot? → A: Guest users have limited access to content, must register to access personalization and chatbot features
- Q: How should the content be organized structurally for the course modules? → A: Create separate chapters for each major module (ROS2, Gazebo/Unity, NVIDIA Isaac, VLA) with integrated assessments and hardware specs in relevant chapters
- Q: How should the "Embodied Intelligence" theme be integrated throughout the content? → A: Make "Embodied Intelligence" the overarching framework connecting all modules, with each chapter showing how the specific technology contributes to embodied AI systems
- Q: How should hardware specifications be integrated with the content? → A: Integrate hardware discussions within each relevant technology chapter, showing how specific hardware applies to each module
- Q: What should be the primary focus for Claude Subagents/Skills? → A: Focus subagents/skills on technical assistance (ROS code generation, simulation setup, hardware configuration) that directly supports the course content
...
As a student learning Physical AI concepts, I want to access a Docusaurus-based textbook with limited content available as a guest, but must register to access the integrated RAG chatbot that answers questions based only on selected text, so I can get immediate clarification on complex topics without external distractions.
...
1. **Given** an authenticated user has selected text in a textbook chapter, **When** they ask the chatbot a question about that text, **Then** the chatbot provides an answer based only on the selected text content
2. **Given** an authenticated user has selected text in a textbook chapter, **When** they ask the chatbot a question unrelated to the selected text, **Then** the chatbot indicates that the question cannot be answered based on the selected text
3. **Given** an authenticated user is viewing a textbook chapter, **When** they haven't selected any text, **When** they ask the chatbot a question, **Then** the chatbot prompts them to select text first
4. **Given** a guest user attempts to use the chatbot, **When** they try to ask a question, **Then** the system prompts them to register first
...
### User Story 3 - Technical Assistance with AI Tools (Priority: P3)

As a student working with Physical AI concepts, I want to access specialized AI subagents/skills for technical assistance (ROS code generation, simulation setup, hardware configuration), so I can get practical help implementing the concepts I'm learning.

**Why this priority**: This addresses the bonus requirement (+50 points) for reusable Claude Code Subagents & Agent Skills providing technical assistance.

**Independent Test**: Can be fully tested by implementing specialized AI subagents/skills for ROS code generation, simulation setup, and hardware configuration, delivering practical implementation assistance.

**Acceptance Scenarios**:

1. **Given** a user is studying ROS concepts in a chapter, **When** they invoke the ROS Code Generation skill, **Then** appropriate ROS code is generated based on their requirements
2. **Given** a user is learning about simulation in a chapter, **When** they invoke the Simulation Setup skill, **Then** appropriate simulation configuration guidance is provided
3. **Given** a user is studying hardware configuration in a chapter, **When** they invoke the Hardware Setup skill, **Then** appropriate hardware configuration instructions are provided

---

### User Story 4 - Multilingual Learning Support (Priority: P4)
...
### User Story 5 - Comprehensive Content Access (Priority: P5)

As a student studying Physical AI, I want access to comprehensive textbook content organized in separate chapters for each major module (ROS2, Gazebo/Unity, NVIDIA Isaac, VLA) with "Embodied Intelligence" as the overarching framework connecting all modules, showing how each technology contributes to embodied AI systems, with hardware specifications (RTX workstation, Jetson kit, robots like Unitree Go2/G1, cloud options) integrated within each relevant technology chapter, plus integrated assessments, weekly learning plans and capstone project, so I can learn all required topics systematically.
...
- How does the system handle large volumes of selected text? If user selects more than 5000 characters, the system truncates to first 5000 characters and displays notification to user, then processes the truncated text without performance degradation.
...
- **FR-008**: System MUST include comprehensive content organized in separate chapters for each major module (ROS2, Gazebo/Unity, NVIDIA Isaac, VLA) with "Embodied Intelligence" as the overarching framework connecting all modules, showing how each technology contributes to embodied AI systems, with hardware specifications (RTX workstation, Jetson kit, robots like Unitree Go2/G1, cloud options) integrated within each relevant technology chapter, plus integrated assessments, weekly plans and capstone project
- **FR-009**: System MUST deploy to GitHub Pages or Vercel for public access with continuous integration/delivery pipeline
- **FR-010**: System MUST include at least 5 reusable Claude Code Subagents and 5 Agent Skills for bonus points (+50 points total); reusable means each subagent/skill can be invoked across multiple chapters/modules and serves different user types/backgrounds; focus on technical assistance (ROS code generation, simulation setup, hardware configuration) that directly supports the course content
...
- **FR-016**: System MUST provide real-time translation API integration for Urdu language support with quality assurance (BLEU score ≥ 0.7 as the primary metric, with human evaluation by 3+ native Urdu speakers during development phase)
- **FR-017**: System MUST store and retrieve textbook content in Qdrant Cloud Free vector database for RAG functionality
- **FR-018**: System MUST implement at least 5 reusable Claude Code Subagents that can be invoked across multiple chapters/modules: ROS Code Generation Subagent, Simulation Setup Subagent, Hardware Configuration Subagent, Technical Troubleshooting Subagent, Implementation Guidance Subagent
- **FR-019**: System MUST include at least 5 reusable Claude Code Agent Skills that can be invoked across multiple chapters/modules: ROS Code Generation Skill, Simulation Configuration Skill, Hardware Setup Skill, Technical Problem Solving Skill, Implementation Assistance Skill
- **FR-020**: System MUST provide responsive UI/UX that works across desktop and mobile devices
- **FR-021**: System MUST implement authentication requirements: limited textbook content available to guests, full content and all features (personalization and chatbot) require user authentication
...
- **SC-003**: Users can switch between English and Urdu content in under 3 seconds with 90% translation quality (measured by BLEU score ≥ 0.7)
...
- **SC-014**: Urdu translation functionality provides accurate translations with 90% quality score (BLEU ≥ 0.7)
...
- **TC-002**: Selected text for chatbot queries limited to maximum 5000 characters; if user selects more than 5000 characters, system truncates to first 5000 characters and displays notification to user
